...
It is not easy to motivate why one would define such a sequence of random variables without some knowledge of the _Replica Method_ and the introduction of the _Replica-Symmetry Breaking_ ansatz that ensues. The goal of the method ends up being to evaluate a certain modified form of a moment of the free energy density of the model under consideration assuming some form of "weights" on overlaps between configurations of independent copies of the model. This generates an overlap matrix, on which a very **specific** structure is imposed through the RSB ansatzen - It is this specific structure that (after some massaging) looks like some smoothed saddle-point equations that are made tractable via the ansatz that give rise to family of parameters that very closely resemble the Parisi Sequence defined above.
