...
Given $$n^2$$ i.i.d. standard normal ($$\mathcal{N}(0, 1)$$) variables $$\{J_{ij}\}_{i, j \in [n]}$$, we are interested in the optimal value of the following optimization problem over the hypercube, <br />
$$ \max_{\sigma \in \{\pm 1\}^n} \sum_{i, j=1}^n J_{ij}\sigma_i\sigma_j\, . $$ 
<br/ > The problem above can be seen as asking for the $$\mathsf{MAX}$$-$$\mathsf{CUT}$$ of a complete graph with i.i.d. $$\mathcal{N}(0,1)$$ weights.
<br />
