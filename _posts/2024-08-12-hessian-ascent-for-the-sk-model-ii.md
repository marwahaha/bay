...
This is the second blog post in a **3-part** series on a recent result [[JSS24]](https://arxiv.org/abs/2408.02360) by [David](https://davidjekel.com/), [Jonathan](https://jshi.science/) and me. The first blog post can be found [here](https://juspreetsandhu.me/2024/08/08/hessian-ascent-for-the-sk-model-i).
...
From the ingredients above, the amount of energy gained in each step follows tediously (but conceptually relatively straightforwardly) by doing a Taylor expansion of the modified objective. The critical observation is to note that the second-order space term is made to interact with the first-order time term in the expansion. In doing the analysis, various terms pop up. Each term needs careful handling and is estimated by using the primal Parisi PDE expansion on the first-order time derivative, followed by invocations of various Gaussian concentration arguments, estimates using the convergence to the primal AC SDE, and the "large-eigenspace" alignment of the entropy corrected Hessian. The complete details are relegated to [[Section 6, JSS24]](https://arxiv.org/pdf/2408.02360) in the paper.
