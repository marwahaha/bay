...
We begin by introducing relevant past work: the Sherrington-Kirkpatrick model and the Parisi formula and their analyses via the Auffinger-Chen stochastic differential equation (SDE) and the generalized TAP free energy. We then sketch the derivation of the algorithm from the generalized TAP free energy, and overview the development of the _primal_ theory for the Parisi PDE (and Auffinger-Chen SDE) that is used heavily in the proofs of the two main theorem statements the algorithm relies on. In later posts we will give a sketch of our proof that Hessian ascent maximizes the generalized TAP free energy by proving these two main statements, and discuss connections to other work and open questions.  
