...
1. Explicitly analyze the expected value (under the algorithm's randomness) of $$\Lambda(t,\sigma_t) $$ where $$t = q^*_\beta $$, such that $$q^*_\beta \to 1 $$ sufficiently fast as $$\beta \to \infty $$ , and
2. Demonstrate that, under fRSB[^2], a particular term that comes from this Parisi PDE in the space "dual" to $$x $$ and (roughly) measures the "difference in length of $$\sigma $$ corrected by a small regularization factor from the expected length of $$\sigma $$", is small. Specifically, we need to show that,
